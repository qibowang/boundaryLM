	...
	public static void main(String[] args) {
		...
		for(int i=0;i<args.length;i++){
			if(args[i].equals("-seg")){
				segPath=args[++i];
				System.out.println("segPath--->"+segPath);
			}else if(args[i].equals("-sep")){
				sepPath=args[++i];
				System.out.println("sepPath--->"+sepPath);
		...
		try {
			...
			MultipleInputs.addInputPath(middleBackJob, new Path(sepPath), SequenceFileInputFormat.class,MiddleBackMapper.class);
			MultipleInputs.addInputPath(middleBackJob, new Path(segPath), SequenceFileInputFormat.class,MiddleBackSEGMapper.class);
